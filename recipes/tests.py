...
class FavoriteFilterTests(TestCase):
    ...
    def add_filter(self):
        model = ContentType.objects.get_for_model(Recipe)

        favorite_filter = FavoriteFilter(
            name="Cake Filter", filtered_model=model, query_parameters='name__icontains="cake"')
    ...
    def test_add_favorite(self):
        ...
        filtered_model = ContentType.objects.get_for_model(Recipe)
        add_favorite_url = f"{add_favorite_url}?filtered_model={filtered_model.id}&query_parameters='name__icontains='chocolate''"
